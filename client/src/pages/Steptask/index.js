...
export default class Steptask extends Component {
    ...
    saveTask() {
        ...
        //初始化页面配置-取油田配置
        this.props.updateBindingData('saveTask', {url: '/' + config.urlSuffix + '/api/saveTask', data: this.state.params}, (item) => {
    ...
    componentDidMount() {
        ...
        //初始化页面配置-取模型配置
        this.props.updateBindingData('getModel', {url: '/' + config.urlSuffix + '/crud/model_config/read', data: {status: "1",pagesize:100}}, (item) => {
            let list = item.data.dataList
            list.map(data => {
    ...
    urlparms = (modellist) => {
        ...
        if (urlparams['id'] != '' && urlparams['id'] != undefined) {
            ...
            this.props.updateBindingData('getTask', {url: '/' + config.urlSuffix + '/api/findTask', data: filter}, (item) => {
                let stepDisplay = this.state.stepDisplay;
    ...
    handleChangeSingle = (value, actionType, item) => {
        ...
        this.props.updateBindingData('findByTaskDepth', {url: '/' + config.urlSuffix + '/api/findByTaskDepth', data: filter}, (item) => {
    ...
    handleQXChange() {
        ...
        // console.log(filter)
        this.props.updateBindingData('findQxByTask', {url: '/' + config.urlSuffix + '/api/findQxByTask', data: filter}, (item) => {
            // console.log(item)
    ...
    inputchange=(rowvalue,value)=>{
        ...
        let tmp={}
        tmp[rowvalue["_id"]]=parseFloat(value)

        _this.datalistModify.push(tmp)
        ...
        return (
            <IceContainer title="测井解释任务步骤" className="adaptationContainer">
                <Step shape="arrow" current={currentStep}>
                    <StepItem title="步骤1:任务信息"/>
                    <StepItem title="步骤2:数据加载"/>
                    <StepItem title="步骤3:数据检查"/>
                    ...
                    <IcePanel style={{height: '400px', 'overflow-x': 'auto'}}>
                        ...
                        <IcePanel.Body>
                            {loglist.map((item, index) => {
                                return (
                                    <Message title={item.title} type={item.type} style={{marginBottom: '10px'}}
                                             key={index}>
                                        {item.content}
                                        {/*{item.hasOwnProperty('diffstr') ? item.content+",不匹配的列:"+item.diffstr : item.content}*/}
                                                   ...
                                                   dataSource={this.state.detail.dataSource}>
                                                {
                                                    this.state.tabelcolumns.map((item, index) => {
                                                        if (item=="modify"){
                                                            ...
                                                            return (
                                                                <Table.Column key={index} title={item} dataIndex={item}/>
                                                            );
                                                        }
                ...
                <div className={styles.buttonGroup}>
                    <ButtonGroup>
                        ...
                        <Button onClick={this.next} type="primary" disabled={currentStep === 4}>下一步</Button>
                    </ButtonGroup>
                </div>
            </IceContainer>
        );
