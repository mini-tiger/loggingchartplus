...
export default class RealTimeStatistics extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      jhcount:0, // 测井数
      qxwjmcount:0, //测井曲线
      taskcount:0, // task status :3
      templatecount:0,
      piccount:0,
      missioncount:0 // all task
    };
  ...
  fetchData=()=>{
    ...
    this.props.updateBindingData('getcount', {url:url,data: {tablename:"task",filter:{status:"3"}}}, (item) => {
  ...
  render() {
    return (
      ...
      <Row wrap gutter="20">
        ...
        <Col xxs="24" s="12" l="8">
          <div className={styles.itemBody6}>
            <div className={styles.itemTitle}>
              <p className={styles.titleText}>任务数</p>
              {/*<span className={styles.tag}>实时</span>*/}
            </div>
            <div className={styles.itemRow}>
              <h2 className={styles.itemNum}>{this.state.missioncount}</h2>
              <div>
                {/*<h2 className={styles.itemNum}>0</h2>*/}
                {/*<p className={styles.desc}>本周新增</p>*/}
              </div>
