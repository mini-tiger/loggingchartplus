...
def getqxwjm():
    txt  = ''
    for i in range(len(qxwjm)-1):
        txt += qxwjm[i] + "','"
    txt += qxwjm[len(qxwjm)-1]
    txt="('"+txt+"')"
    return txt
'''
...
def initDB(userName, passWord, ip, port, sid):
    ...
    sqlStr = 'SELECT %s from WL18 where JH in %s AND QXWJM in %s order by QXWJM'%(getCols(),getjh(),getqxwjm())
    print(sqlStr)
    ...
    for i in range(total_qx):
        ...
        if i==(total_qx-1) or jh != curJH or lx!=curLx or (tDepth[0]!=d1 or tDepth[1]!= d2):  # 有一个新的‘井号’或者‘深度区间’，则建立一个新的文件
            if len(logNamelist) > 0:
                ...
                #print('Write END')
                lasCount += 1
                print('lasCount = %s,  write las: well = %s,  orignal file = %s, rowIndex = %s'%(lasCount, curJH, lx, i))
                logNamelist.clear()
                logDatalist.clear()
            curJH = jh
            curLx = lx
        ...
        for j in range(0, min(samples,len(tLog)) ):
            try:
                ...
                tData[j] = -999.25
                tmpErr = '[errorIndex = %s]  i = %s, well = %s, tLog[j] = %s(j=%s, samples=%s), tLog[last] = %s '%(errorIndex, i, curJH, tLog[j], j, samples, tLog[len(tLog)-1])
                print('# # # ', tmpErr)
                fWrite.write(tmpErr + '\n')
...
def get_res_data(allFieldlist):
    ...
    save_data(result1,allFieldlist)
# 保存文件
def save_data(res,allFieldlist):
...
if __name__ == '__main__':
    ...
    if os.path.exists(outPath) == False:
        os.mkdir(outPath)
    cursor = None
    allFieldlist=initDB(userName, passWord, ip, port, sid)
    get_res_data(allFieldlist)
    shutil.rmtree('newLogPG')
