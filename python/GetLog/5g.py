...
options, args = getopt.getopt(sys.argv[1:], 'u:j:q:')   
print(options)
...
def initDB(userName, passWord, ip, port, sid):
    ...
    sqlStr = 'SELECT %s from WL18 where JH in %s AND ZBMC in %s order by QXWJM'%(getCols(),getjh(),getqx())
    print(sqlStr)
...
if __name__ == '__main__':
    ...
    if os.path.exists(outPath) == False:
        os.mkdir(outPath)
    cursor = None
    initDB(userName, passWord, ip, port, sid)
    get_res_data()
